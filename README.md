...
In the second stage, a new weight function, \( r(x; a) \), was introduced, based on the nonlinear assumption space of the Gaussian kernel function. The final classification function remains within the linear assumption space. The parameter \( a \) in the weight function \( r(x; a) \) can be solved using the **L-BFGS-B** optimisation algorithm. Finally, the combination of mean square loss, logistic loss, and hinge loss, along with the solution of the weight parameter \( \theta \) via the L-BFGS-B optimisation algorithm, demonstrates that the **IWL model** outperforms the unweighted LSC algorithm on the test set. The **SC (Important Weight Least Squares Classification)** algorithm, which incorporates the weight function \( r(x; a) \), has been demonstrated to be more accurate than the unweighted LSC algorithm. This enhanced accuracy is subjected to a comprehensive analysis.
